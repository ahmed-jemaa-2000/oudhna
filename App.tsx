...
export default function App() {
  ...
  const copyright = lang === 'ar' ? 'جميع الحقوق محفوظة لـ نجيبة' : 'All rights reserved to Najiba';
    ...
    <div className={`min-h-screen bg-app-bg text-app-text font-cairo ${lang === 'ar' ? 'rtl' : 'ltr'}`} dir={lang === 'ar' ? 'rtl' : 'ltr'}>
      ...
      <header className="sticky top-0 z-50 bg-app-bg/80 backdrop-blur-md border-b border-app-border transition-colors duration-300">
        <div className="container mx-auto px-4 h-16 flex items-center justify-between">
          <h1 className="text-xl md:text-2xl font-black bg-gradient-to-r from-brand-primary via-brand-secondary to-brand-accent bg-clip-text text-transparent animate-gradient-x">
            نجيبة
          </h1>
          ...
          <div className="flex gap-2 items-center">
            <button
              onClick={toggleTheme}
              className="p-2 rounded-full hover:bg-app-surface transition-colors flex items-center gap-2 text-app-muted hover:text-brand-primary"
            ...
            <div className="w-9 h-9 md:w-10 md:h-10 rounded-full overflow-hidden border-2 border-brand-primary shadow-[0_0_15px_rgba(139,92,246,0.4)] ml-2 transition-transform hover:scale-110 cursor-pointer">
              <img
                src="https://ui-avatars.com/api/?name=Najiba&background=8B5CF6&color=fff&size=128&bold=true"
                alt="Najiba"
      ...
      <main className="container mx-auto px-4 py-8 pb-32">
        ...
        <div className="text-center mb-10 animate-fade-in-up">
          <h2 className="text-3xl md:text-5xl font-black mb-4 leading-tight text-transparent bg-clip-text bg-gradient-to-r from-brand-primary via-brand-secondary to-brand-accent animate-gradient-x">
            {appTitle}
          </h2>
            ...
            {navItems.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`flex flex-col items-center gap-1 transition-all duration-300 w-full ${isActive ? 'text-brand-primary -translate-y-2' : 'text-app-muted hover:text-app-text'}`}
                >
                  <div className={`p-2 rounded-2xl transition-all ${isActive ? 'bg-brand-primary/10 shadow-[0_0_20px_rgba(139,92,246,0.4)]' : ''}`}>
